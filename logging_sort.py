...
class logs:
    def __init__(self, folder_path):
        ...
        with open(self.log_text_file, 'w', encoding="utf-8") as L:
    ...
    def call_to_check_all(self, path):
        t = timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        with open(self.log_text_file, 'a', encoding="utf-8") as L:
    ...
    def stary_files_logs(self, stray_file_list: list):
        t = timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        with open(self.log_text_file, 'a', encoding="utf-8") as L:
            L.write(f"Stray files:  \n")
            for index, item in enumerate(stray_file_list):
                L.write(f"{index+1}. {item}  ---  [{t}]\n")
            L.write("\n")
